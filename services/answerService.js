...
module.exports = {
    AnswerQuestion: async (req, res, next) => {
        try {
            ...
            payload.userId = userId //still to impliment 
            ...
            res.status(201).json({ message: "Thank you for you answer", payload: { data: answered } })
    ...
    updateAnswer: async (req, res, next) => {
        try {
            const { answerId } = req.params;
            const payload = req.body;
            const updatedAnswer = await Answer.findByIdAndUpdate({ _id: answerId }, payload);
            res.status(200).json({ message: "Answer updated successfully", payload: { data: updatedAnswer } })
        } catch (error) {
            next(error)
        }
    },
